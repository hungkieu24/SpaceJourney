...
export default function SpiralImages(props: any) {
    ...
    useEffect(() => {
        ...
        const draw = (now: number) => {
            ...
            progressRef.current = (((progressRef.current + speed * f) % 100) + 100) % 100
            ...
            for (let k = 0; k < cards.length; k++) {
                ...
                const normalizedDist = Math.min(dist / R, 1)
                const scale =
                    sizeAttenuation > 0
                        ? Math.pow(normalizedDist, sizeAttenuation * 0.5) * (1 + 4 * Math.pow(normalizedDist, 3))
                        : 1
                ...
                ctx.save()
                ctx.translate(x, y)
                // ctx.rotate(angle) // Removed to keep images upright
