...
function createVortex(
  ...
  function driveComet(comet: Comet, now: number, dt: number, entrance: number) {
    ...
    for (let i = 0; i < tailLen; i++) {
      ...
      const g = Math.max(0, Math.min(1, 2 - Math.abs(hue * 6 - 2)));
      const b = Math.max(0, Math.min(1, 2 - Math.abs(hue * 6 - 4)));
      comet.trailCol[at] = r * v * hot;
      comet.trailCol[at + 1] = g * v * hot;
      comet.trailCol[at + 2] = b * v * hot;
    }
  ...
  React.useEffect(() => {
    ...
    const attemptInit = () => {
      try {
        ...
        if (retryCount < 3) {
          retryCount++;
          timeoutId = setTimeout(attemptInit, 200);
          return;
        }
        console.warn("[Vortex] init failed after retries:", err);
      }
    };
...
export default function Tornado(props: Record<string, unknown>) {
